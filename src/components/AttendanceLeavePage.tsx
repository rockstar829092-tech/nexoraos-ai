    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-blue-100 selection:text-blue-700 pb-20">
          ...
          <div className="flex items-center gap-3">
             <div className="hidden sm:flex flex-col items-end text-right">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${syncing ? 'bg-blue-500 animate-ping' : 'bg-emerald-500'}`} />
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10 relative z-10">
                {[
                  ...
                  <div key={i} className="p-5 rounded-3xl border border-slate-100 bg-slate-50/50 flex flex-col gap-3 group hover:bg-white hover:shadow-md transition-all">
                    <div className="flex justify-between items-start">
                      <div className="p-2 bg-white rounded-xl shadow-sm border border-slate-100">
                        <Wifi className={`w-4 h-4 ${device.active ? 'text-emerald-500' : 'text-blue-500 animate-pulse'}`} />
                      </div>
              ...
              <div className="bg-[#0F172A] rounded-[24px] p-6 shadow-xl relative mt-4">
                 <div className="flex items-center justify-between mb-4">
                    <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Live Attendance Feed</h5>
                    <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)] animate-pulse" />
                 </div>
                 <div className="space-y-4">
                   {[
                     { time: '08:02 AM', name: 'Aarav Sharma', action: 'Punch In', avatar: 'AS' },
                     { time: '08:05 AM', name: 'Priya Verma', action: 'Punch In', avatar: 'PV' },
                     { time: '08:11 AM', name: 'Rahul Singh', action: 'Punch In', avatar: 'RS' },
                     { time: '08:13 AM', name: 'Ananya Gupta', action: 'Punch In', avatar: 'AG' },
                   ].map((feed, i) => (
                     <div key={i} className="flex items-center justify-between group">
                        <div className="flex items-center gap-3">
                           <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-[10px] font-bold text-slate-400">{feed.avatar}</div>
                           <div>
                              <p className="text-xs font-black text-slate-200 group-hover:text-blue-400 transition-colors">{feed.name}</p>
                              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">{feed.time}</span>
                           </div>
                        </div>
                        <span className="px-3 py-1 bg-blue-500/10 text-blue-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-blue-500/20">{feed.action}</span>
                     </div>
                   ))}
                 </div>
              </div>
            ...
            <section className="bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm">
              ...
              <div className="space-y-4">
                 {[
                   ...
                   <div key={i} className="p-6 border border-slate-100 rounded-[28px] bg-slate-50/30 group hover:bg-white hover:border-slate-200 hover:shadow-md transition-all">
                      <div className="flex flex-col sm:flex-row justify-between gap-6">
                        <div className="flex gap-4 flex-1">
                           <div className={`w-14 h-14 rounded-2xl ${req.bg} flex items-center justify-center text-xl font-black ${req.color} shadow-sm border border-slate-100`}>
                             {req.name.charAt(0)}
                           </div>
                      ...
                      <div className="mt-6 p-4 bg-white border border-slate-100 border-dashed rounded-2xl">
            ...
            <section className="bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-2">
                   <button className="p-2 border border-slate-100 rounded-xl hover:bg-slate-50 text-slate-400 cursor-pointer"><Download className="w-4 h-4" /></button>
                   <button className="p-2 border border-slate-100 rounded-xl hover:bg-slate-50 text-slate-400 cursor-pointer"><Printer className="w-4 h-4" /></button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                 {[
                   ...
                   <div key={i} className="p-5 border border-slate-100 bg-slate-50/50 rounded-3xl hover:bg-white hover:shadow-md hover:border-slate-200 transition-all">
              ...
              <div className="flex justify-between items-center p-4 bg-slate-900 rounded-2xl">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <h5 className="text-[11px] font-black text-white uppercase tracking-widest">Aggregate Compliance</h5>
                       <p className="text-[10px] text-slate-500 font-bold">Calculated across 4,200 unique records</p>
                    </div>
                 </div>
                 <div className="text-right">
                    <span className="text-xl font-black text-blue-400 font-mono tracking-tighter">95.4%</span>
                    <span className="text-[9px] text-slate-600 block font-bold uppercase">Consistency Index</span>
                 </div>
              </div>
            </section>
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`h-12 border border-slate-50 rounded-xl flex flex-col items-center justify-center gap-1 group relative transition-all cursor-pointer ${
                        ...
                        status === 'absent' ? 'bg-rose-50/30' :
                        status === 'leave' ? 'bg-amber-50/30' : 'bg-blue-50/30 font-bold opacity-40'
                      }`}
                    >
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="sticky top-28 space-y-8">
              <motion.div 
                ...
                className="bg-[#0F172A] rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl shadow-blue-900/20"
              >
                {/* Visual Decorations */}
                <div className="absolute top-0 right-0 w-80 h-80 bg-blue-600/20 rounded-full blur-[120px] -z-0 pointer-events-none" />
                <div className="absolute inset-0 bg-[radial-gradient(#ffffff0a_1.5px,transparent_1.5px)] bg-[size:30px_30px]" />

                <div className="relative z-10 space-y-8">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-[0.3em] text-blue-400 mb-2">Admin Dashboard</h4>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         <span className="text-[9px] font-black text-emerald-400 uppercase">Live Stats</span>
                      </div>
                    </div>
                    ...
                    <div className="h-56 w-full relative">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none translate-y-1">
                         ...
                         <span className="text-3xl font-black text-white font-mono tracking-tighter leading-none">96.2%</span>
                         <span className="text-[10px] font-bold text-blue-400 uppercase mt-1">Present</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      {ATTENDANCE_BREAKDOWN.map((item, i) => (
                        <div key={i} className="flex flex-col gap-1 p-3 bg-white/5 border border-white/5 rounded-2xl group hover:bg-white/10 transition-all">
                           <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { label: "Pending Leave Requests", val: "18", color: "text-amber-400", icon: Mail },
                      { label: "Late Arrivals Detected", val: "27", color: "text-purple-400", icon: Clock },
                      { label: "Active Biometric Units", val: "12 / 12", color: "text-emerald-400", icon: Wifi },
                      { label: "Avg Attendance (Mo.)", val: "95.4%", color: "text-blue-400", icon: TrendingUp },
                    ].map((kpi, i) => (
                      <motion.div 
                        key={i} 
                        whileHover={{ y: -5 }}
                        className="bg-white/5 backdrop-blur-md border border-white/10 p-5 rounded-[28px] group"
                      >
                         <div className={`p-2 bg-white/5 w-fit rounded-xl mb-4 ${kpi.color}`}>
                           <kpi.icon className="w-4 h-4" />
                         </div>
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1 group-hover:text-slate-300 transition-colors">{kpi.label}</span>
                         <h5 className="font-black text-xl tracking-tighter text-white">{kpi.val}</h5>
                      </motion.div>
                    ))}
                  </div>
                       ...
                       {[
                         ...
                         <div key={i} className="p-4 bg-white/5 border border-white/5 rounded-2xl flex items-center justify-between group hover:bg-white/10 transition-all">
                            <div className="flex items-center gap-3">
                               ...
                               <span className="text-[10px] font-bold text-slate-400 group-hover:text-slate-100 transition-colors uppercase tracking-tight">{insight.label}</span>
                            </div>
                            <div className="flex items-center gap-2">
                               <span className="text-sm font-black text-white font-mono">{insight.val}</span>
                               <div className="h-8 w-12 pt-2">
                                  <ResponsiveContainer width="100%" height="100%">
                                     <AreaChart data={WORKFORCE_TRENDS}>
                                        <Area type="monotone" dataKey={i % 2 === 0 ? 'teaching' : 'nonTeaching'} stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.1} />
                                     </AreaChart>
                                  </ResponsiveContainer>
                               </div>
                            </div>
                      ...
                      {[
                        ...
                        <div key={i} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-3xl group hover:bg-white/10 transition-all cursor-pointer">
                           <div className="flex-1">
                              <p className="text-[11px] font-bold text-slate-100 leading-snug group-hover:text-blue-400 transition-colors">{activity.text}</p>
                              <div className="flex items-center gap-2 mt-1">
                                <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded shadow-sm ${
                                   ...
                                   activity.status === 'Synced' ? 'bg-blue-500/20 text-blue-400' :
                                   activity.status === 'Generated' ? 'bg-purple-500/20 text-purple-400' : 'bg-rose-500/20 text-rose-400'
                                }`}>{activity.status}</span>
                                <span className="text-[9px] text-slate-600 font-bold tracking-tight">• {activity.time}</span>
                              </div>
                           </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
         <motion.button 
           ...
           whileTap={{ scale: 0.95 }}
           className="flex items-center gap-3 px-8 py-4 bg-[#0F172A] text-white rounded-full shadow-2xl shadow-blue-900/40 border border-white/10 cursor-pointer"
         >
